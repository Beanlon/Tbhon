...
import {
  SPUTUM_DEFER_MODAL_MESSAGE,
  SPUTUM_DEFER_MODAL_TITLE,
  SPUTUM_DEFER_REASON_MAX_LENGTH,
  SPUTUM_DEFER_REASON_PLACEHOLDER,
  SPUTUM_DEFER_REASON_SUGGESTIONS,
...
export function SputumDeferReasonModal({ visible, onCancel, onConfirm }: Props) {
  ...
  const handleConfirm = () => {
    ...
    onConfirm(trimmed.slice(0, SPUTUM_DEFER_REASON_MAX_LENGTH));
  };
        ...
        >
          <View className="mb-4 flex-row items-start justify-between gap-3">
            <View className="min-w-0 flex-1">
              <Text className="text-lg font-bold text-white">{SPUTUM_DEFER_MODAL_TITLE}</Text>
              <Text className="mt-2 text-sm leading-5 text-white/65">{SPUTUM_DEFER_MODAL_MESSAGE}</Text>
            </View>
            <Pressable
              onPress={onCancel}
          ...
          >
            {SPUTUM_DEFER_REASON_SUGGESTIONS.map((suggestion) => (
              <Pressable
                key={suggestion}
          ...
          <Pressable
            ...
            accessibilityRole="button"
          >
            <Text className="text-sm font-semibold text-white/55">Cancel — return to smear options</Text>
          </Pressable>
        </Pressable>
      </Pressable>
    </Modal>
